...
public class Book {
    ...
    public static Boolean isTitleWord(String word) {
        ...
        String[] arr = {"a", "an", "the", "and", "but", "for", "nor", "or", "so", "yet",
                "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
    ...
    public void setTitle(String title) {
        ...
        this.title = capitalize(titleArr[0]);
        for (int i=1; i<titleArr.length; i++) {
          if (isTitleWord(titleArr[i])) {
            this.title += " " + capitalize(titleArr[i]);
          }
          else {
            this.title += " " + titleArr[i].toLowerCase();
          }
        }
    }
     */
    ...
    public void setAuthor(String author) {
        if (author.isEmpty()) {
            ...
            this.author = capitalize(splittedAuthor[0]);
            for (int i = 1; i < splittedAuthor.length; i++) {
                this.author += " " + capitalize(splittedAuthor[i]);
            }
        }
    }
    ...
    public void setTitle(String title) {
        if (title.length() == 0) {
            ...
            String s = capitalize(splittedTitle[0]);
            for (int i = 1; i < splittedTitle.length; i++) {
                if (isTitleWord(splittedTitle[i]))
                    s += " " + capitalize(splittedTitle[i]);
                else
                    s += " " + splittedTitle[i].toLowerCase();
                this.title = s;
            }
        }
    }
